...
public abstract class WaCuratorFactory {

	private static final Log log = LogFactory.getLog(WaCuratorFactory.class);
...
//				final String beanName = "configDataCuratorFramework";
//				if (!event.getApplicationContext().getBeanFactory().containsBean(beanName)) {
//					event.getApplicationContext().getBeanFactory().registerSingleton(beanName,
//							curatorFramework);
//				}
//			}
//		});
	...
	private static CuratorFramework curatorFramework(BootstrapContext context, ZookeeperProperties properties,
		...
		try {
			return WaCuratorFactory.curatorFramework(properties, context.get(RetryPolicy.class), customizers,
